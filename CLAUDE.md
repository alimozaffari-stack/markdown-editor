...
Markdown Editor is a cross-platform Markdown editing application for macOS, Windows and Linux, built with Tauri v2 (Rust backend) + React/TypeScript/Tailwind (frontend) + TipTap (WYSIWYG editor) + Tantivy (full-text search).
...
- Settings live in two places: app config at `{APP_DATA}/config.json`, and compatible per-folder settings at `{NOTES_FOLDER}/.scratch/settings.json`.
